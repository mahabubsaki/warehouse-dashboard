...
const Returned = () => {
    ...
    useEffect(() => {
        ...
        try {
            async function fs() {
                const newData = await fetchdata(user.role == 'admin' ? `get-returned-list?page=${activePage}&email=${user.email}&status=No` : `get-returned-list?page=${activePage}&warehouse=${user.warehouse}&status=No`, axiosInstance);
