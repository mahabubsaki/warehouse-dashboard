...
const MissingItemsSolved = () => {
    ...
    useEffect(() => {
        async function fs() {
            const newData = await fetchdata('get-missing?status=Solved', axiosInstance);
            setSolved(newData);
        }
        fs();
    }, []);
    return (
        <div>
            <div>
                <h1 className='text-3xl text-center my-8'>Total Missing Items : {currentData.totalProducts}</h1>
            </div>
