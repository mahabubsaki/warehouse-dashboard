...
const LastSell = () => {
    ...
    useEffect(() => {
        ...
        try {
            async function fs() {
                const newData = await fetchdata(user.role == 'admin' ? `get-month-sell?status=Shipped&page=${activePage}&month=last&email=${user?.email}&search=${search}` : `get-month-sell?status=Shipped&page=${activePage}&month=last&warehouse=${user?.warehouse}&search=${search}`, axiosInstance);
